...
%%测试玩个球
playaball(Socket) ->
	L = byte_size( <<1:16,40000:16,1:16>>),
	gen_tcp:send(Socket, <<L:16,40000:16,1:16>>),
	rec(Socket).

%%测试玩个球结果
playballresult(Socket) ->
	L = byte_size( <<1:16,40001:16,120:8,4:8,15:8,3:8,20:8,41:8,30:8,6:8,3:8,40:8,37:8,2:8,41:8,40:8,101:8,7:8,41:8,42:8>>),
	gen_tcp:send(Socket,<<L:16,40001:16,120:8,4:8,15:8,3:8,20:8,41:8,30:8,6:8,3:8,40:8,37:8,2:8,41:8,40:8,101:8,7:8,41:8,42:8>>),
	rec(Socket).
...
%%创建角色
create(Socket) -> 
    L = byte_size( <<1:16,10003:16,1:8,1:8,1:8,7:16,"逍遥123">>),
    gen_tcp:send(Socket, <<L:16,10003:16,1:8,1:8,1:8,7:16,"逍遥123">>),
...
rec(Socket) ->
    receive
		...
		%%测试网关
		{tcp,Socket,<<_L:16,60000:16,Bin/binary>>}->
			<<L:16,Bin2/binary>> = Bin,
			F = fun(Bin3) ->
				<<Id:8,Data1/binary>> = Bin3,
				{Ip,Bin4} = read_string(Data1),
				<<Port:16, State:8, Num:16>> = Bin4,
				io:format("Recv server info：~p~n", [[Id,Ip,Port,State,Num]])
			end,
			for(0,L,F,Bin2);
		
		%%投个篮
		{tcp,Socket,<<_L:16,40000:16,Time:8,Bin/binary>>}->
			<<L:8,Bin2/binary>> = Bin,
			io:format("pt 40000 Time result:~p ~p ~p~n", [Time,L,[Bin2]]);
			%F = fun(Bin3) ->
			%	<<TimeStap:8,Res1:8,Res2:8,Res3:8,Bin4/binary>> = Bin3,
			%	io:format("pt 40000 array result:~p~n", [[TimeStap,Res1,Res2,Res3]]),
			%	Bin4
			%end,

		%%投个篮结果
		{tcp,Socket,<<_L:16,40001:16,RetCode:8>>}->
			io:format("The RetCode is ~p~n!",[RetCode]),
			case RetCode of
				0 ->
					io:format("The Result is Right!");
				1 ->
					io:format("The Result is wrong!")
			end;
